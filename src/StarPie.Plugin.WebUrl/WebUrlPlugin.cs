...
/// StarPie 随包动作包「打开网址」。
/// <para>
/// 它认领两个顶层动作类型：<c>WebUrl</c> 与历史别名 <c>Url</c> ——
...
/// 停用其中一个只失效一半配置，而用户完全看不出另一半为什么还活着。
/// </para>
/// <para>
/// <b>单动作包的意义</b>：停用粒度精确到一个动作。它与「启动程序」「运行命令」等
/// 原本同属一个基础动作包，要关就得五个一起关。
/// </para>
/// </summary>
public sealed class WebUrlPlugin : IStarPiePlugin
{
    ...
    public void Initialize(IPluginContext context)
    {
        ...
        //    两个类型名（WebUrl / Url）指向的是同一个短 ID `webUrl`。
        context.Actions.Register(new WebUrlAction(context));

        context.Log.Info("打开网址包已就绪");
    }
